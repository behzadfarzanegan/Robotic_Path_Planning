...
DijkstraPlanner::DijkstraPlanner(): Node("dijkstra_planner")
{
    ...
    nav_msgs::msg::Path DijkstraPlanner::plan(const geometry_msgs::msg::Pose & start, const geometry_msgs::msg::Pose & goal)
    {
        ...
        while(!pending_node.empty() && rclcpp::ok()){
            ...
            if (visited_map_.data.at(poseToCell(active_node)) == 100) continue;
            
            // Mark as Closed
            visited_map_.data.at(poseToCell(active_node)) = 100;
            ...
            for(const auto & dir: explore_directions){
                ...
                if(poseOnMap(new_node) 
                   && visited_map_.data.at(poseToCell(new_node)) != 100
...
int main(int argc, char ** argv)
{
    ...
    auto planner_node = std::make_shared<bumperbot_planning::DijkstraPlanner>();
